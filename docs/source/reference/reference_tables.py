...
from amocatlas.defaults import ARRAY_NAMES, ARRAY_DESCRIPTIONS
...
def generate_variable_mapping_table(mappings):
    ...
    for file_key, info in mappings.items():
        ...
        if len(filename) > 45:
            ...
            if 'VOLUMETRANSPORT' in filename:
                ...
                filename = filename.replace('CURRENTS-AT-SITES', 'CURRENTS-AT-SITES')  # Already has hyphens
    ...
    sorted_mappings = sorted(all_mappings, 
                           key=lambda x: (x['standardized_name'], x['dataset_file']))
...
def generate_standardized_variables_table(standardized_vars):
    ...
    for var_name in sorted(standardized_vars.keys()):
        ...
        descriptions = [inst['description'] for inst in instances if inst['description']]
        units_list = [inst['units'] for inst in instances if inst['units']]
        standard_names = [inst['standard_name'] for inst in instances if inst['standard_name']]
        ...
        vocabulary_url = ""
        if standard_name:
            if any(name in standard_name.lower() for name in ['ocean_', 'sea_water_', 'northward_', 'transport']):
                vocabulary_url = "`CF-1.8 <https://cfconventions.org/Data/cf-standard-names/current/>`_"
            elif 'Transport_anomaly' in standard_name or 'Heat_transport' in standard_name or 'Freshwater_transport' in standard_name:
