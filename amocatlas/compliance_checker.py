...
GENERIC_SPECS = {
    ...
    'required_global_attrs': [
        'Conventions', 'format_version', 'data_type', 'featureType', 'data_mode',
        'title', 'summary', 'source', 'site_code', 'array',
        'geospatial_lat_min', 'geospatial_lat_max', 'geospatial_lon_min', 'geospatial_lon_max',
        'time_coverage_start', 'time_coverage_end', 'contributor_name', 'contributor_role',
        'contributor_role_vocabulary', 'contributor_id', 'contributing_institutions',
        'contributing_institutions_vocabulary', 'contributing_institutions_role',
        'contributing_institutions_role_vocabulary', 'source_acknowledgement',
        # Additional mandatory OceanSITES attributes
        'platform_code', 'naming_authority', 'id', 'cdm_data_type', 'QC_indicator',
        'processing_level', 'date_created', 'institution'
    ],
...
RAPID_SPECS = {
    ...
    'file_requirements': {
        'component_transports': {
            ...
            'required_variables': ['TRANSPORT', 'MOC_TRANSPORT', 'TRANSPORT_NAME', 'TRANSPORT_DESCRIPTION']
        },
        'meridional_transports': {
            'dimensions': {'DEPTH': 307, 'SIGMA0': 631},
            'required_variables': ['MOC_TRANSPORT_DEPTH', 'MOC_TRANSPORT_SIGMA', 'HEAT_TRANSPORT', 
                                 'FRESHWATER_TRANSPORT', 'STREAMFUNCTION_DEPTH', 'STREAMFUNCTION_SIGMA']
        },
        'streamfunction': {
...
class AC1ComplianceChecker:
    ...
    def _validate_filename(self, filepath: str, result: ValidationResult) -> Optional[str]:
        """Validate filename against OceanSITES convention."""
        filename = Path(filepath).name
        
        # Check general OceanSITES pattern first
        general_match = re.match(self.generic_specs['general_filename_pattern'], filename)
        if not general_match:
            result.errors.append(f"Filename must follow OceanSITES pattern: OS_[PSPANCode]_[StartEndCode]_[ContentType]_[PARTX].nc")
            return None
        ...
        if pspan_code != self.array_specs['site_code']:
            result.errors.append(f"PSPANCode must be '{self.array_specs['site_code']}' for {self.array_specs['site_code']} data, got '{pspan_code}'")
            return None
        
        # Validate ContentType
        if content_type not in self.generic_specs['valid_content_types']:
            result.errors.append(f"ContentType must be one of {self.generic_specs['valid_content_types']}, got '{content_type}'")
            return None
        ...
        try:
            ...
            if start_dt > end_dt:
                result.errors.append(f"Start date ({start_date}) must be <= end date ({end_date})")
        ...
        if not file_type:
            ...
            result.warnings.append(f"Using generic file type '{file_type}' - no specific pattern matched")
        
        return file_type
    
    def _validate_dimensions(self, ds: xr.Dataset, file_type: str, result: ValidationResult):
    ...
    def _validate_array_specific_dimensions(self, ds: xr.Dataset, file_type: str, result: ValidationResult):
        ...
        if 'dimensions' in requirements:
            for dim_name, expected_size in requirements['dimensions'].items():
                if dim_name not in ds.sizes:
                    result.errors.append(f"{dim_name} dimension required for {file_type}")
                elif ds.sizes[dim_name] != expected_size:
                    result.errors.append(f"{dim_name} must be {expected_size}, got {ds.sizes[dim_name]}")
        
        # Check forbidden dimensions
        if 'forbidden_dimensions' in requirements:
            for dim_name in requirements['forbidden_dimensions']:
                if dim_name in ds.dims:
                    result.errors.append(f"{dim_name} dimension should NOT be present in {file_type} files")
            ...
            if comp_idx >= 0:
                for idx in [t_idx, z_idx, y_idx, x_idx]:
                    if idx >= 0 and comp_idx > idx:
                        result.errors.append(f"Variable {var_name}: N_COMPONENT must be leftmost of spatiotemporal dimensions")
                        break
            ...
            if len(indices) > 1:
                for i in range(len(indices) - 1):
                    if indices[i][0] > indices[i+1][0]:
                        result.warnings.append(f"Variable {var_name}: Dimensions not in T,Z,Y,X order")
                        break
    
    def _validate_variables(self, ds: xr.Dataset, file_type: str, result: ValidationResult):
    ...
    def _validate_array_specific_variables(self, ds: xr.Dataset, file_type: str, result: ValidationResult):
        ...
        if 'required_variables' in requirements:
            for var_name in requirements['required_variables']:
                if var_name not in ds.data_vars:
                    result.errors.append(f"Required variable {var_name} missing for {file_type}")
        ...
        for coord_name, coord_spec in self.generic_specs['coordinate_specs'].items():
            if coord_name in ds.coords:
                ...
                if 'axis' in coord_spec:
                    ...
                    if 'axis' not in coord_var.attrs or coord_var.attrs['axis'] != expected_axis:
                        result.errors.append(f"{coord_name} coordinate must have axis='{expected_axis}'")
                
                # Check units attribute
                if 'units' in coord_spec:
                    expected_units = coord_spec['units']
                    if 'units' not in coord_var.attrs:
                        result.errors.append(f"{coord_name} coordinate must have units attribute")
                    elif coord_var.attrs['units'] != expected_units:
                        result.errors.append(f"{coord_name} units must be '{expected_units}', got '{coord_var.attrs['units']}'")
    ...
    def _validate_variable_attributes(self, var_name: str, var: xr.DataArray, result: ValidationResult):
        ...
        for attr in required_attrs:
            if attr not in var.attrs:
                ...
                result.errors.append(f"Variable {var_name} missing required attribute: {attr}")
        
        # Check _FillValue data type
        if '_FillValue' in var.attrs:
            fill_value = var.attrs['_FillValue']
            if not isinstance(fill_value, type(var.values.flat[0])):
                result.warnings.append(f"Variable {var_name}: _FillValue type doesn't match variable dtype")
        ...
        if 'vocabulary' in var.attrs:
            ...
            if standard_name in required_vocabs:
                ...
                if vocab_url != expected_vocab:
                    result.errors.append(f"Variable {var_name}: Wrong vocabulary URL. Expected {expected_vocab}, got {vocab_url}")
        ...
        if 'units' in var.attrs:
            ...
            if var_name == 'TIME' and standard_name == '':
                ...
                if units != approved_units['time']:
                    result.errors.append(f"Variable {var_name}: Non-compliant time units '{units}', expected '{approved_units['time']}'")
                ...
                if coord_key in approved_units and units != approved_units[coord_key]:
                    result.errors.append(f"Variable {var_name}: Non-compliant {coord_key} units '{units}', expected '{approved_units[coord_key]}'")
                ...
                if standard_name in approved_units:
                    ...
                    if units not in acceptable_units:
                        if len(acceptable_units) == 1:
                            result.errors.append(f"Variable {var_name} (standard_name='{standard_name}'): Non-compliant units '{units}', expected '{expected_units}'")
                        else:
                            result.errors.append(f"Variable {var_name} (standard_name='{standard_name}'): Non-compliant units '{units}', expected one of {acceptable_units}")
                
                # If standard_name not in our approved list, just issue a warning
                elif standard_name not in approved_units:
                    result.warnings.append(f"Variable {var_name}: Unknown standard_name '{standard_name}' - cannot validate units '{units}'")
    ...
    def _validate_generic_global_attributes(self, ds: xr.Dataset, result: ValidationResult):
    ...
    def _validate_array_specific_global_attributes(self, ds: xr.Dataset, result: ValidationResult):
        """Validate array-specific global attributes."""
        
        # Check array-specific values
        if 'site_code' in ds.attrs and ds.attrs['site_code'] != self.array_specs['site_code']:
            result.errors.append(f"site_code must be '{self.array_specs['site_code']}' for {self.array_specs['site_code']} data")
        
        if 'array' in ds.attrs and ds.attrs['array'] != self.array_specs['array']:
            result.errors.append(f"array must be '{self.array_specs['array']}' for {self.array_specs['array']} data")
        
        if 'data_mode' in ds.attrs and ds.attrs['data_mode'] != self.array_specs['data_mode']:
            result.errors.append(f"data_mode must be '{self.array_specs['data_mode']}' for {self.array_specs['site_code']} data")
    
    def _validate_additional_global_attributes(self, ds: xr.Dataset, result: ValidationResult):
        ...
        if 'contributor_role_vocabulary' in ds.attrs:
            ...
            if not (vocab_url.startswith('http://') or vocab_url.startswith('https://')):
                result.errors.append(f"contributor_role_vocabulary must be a valid URL, got '{vocab_url}'")
            elif vocab_url != expected_url:
                result.warnings.append(f"contributor_role_vocabulary is non-standard. Expected '{expected_url}', got '{vocab_url}'")
        ...
        if 'contributor_id' in ds.attrs:
            ...
            for id_val in ids:
                ...
                if 'orcid.org' in id_val.lower():
                    if not re.match(r'https://orcid\.org/\d{4}-\d{4}-\d{4}-\d{3}[0-9X]$', id_val):
                        result.errors.append(f"Invalid ORCID format: {id_val}. Must be https://orcid.org/XXXX-XXXX-XXXX-XXXX")
                # For other ID types, just check it's not empty and contains some structure
                elif not id_val or len(id_val.strip()) < 3:
                    result.errors.append(f"contributor_id entries must be meaningful identifiers, got: '{id_val}'")
        ...
        if 'data_mode' in ds.attrs:
            ...
            if ds.attrs['data_mode'] not in valid_data_modes:
                result.errors.append(f"data_mode must be one of {valid_data_modes}, got '{ds.attrs['data_mode']}'")
        ...
        if 'QC_indicator' in ds.attrs:
            ...
            if ds.attrs['QC_indicator'] not in valid_qc_indicators:
                result.errors.append(f"QC_indicator must be one of {valid_qc_indicators}, got '{ds.attrs['QC_indicator']}'")
        
        # Check cdm_data_type values
        if 'cdm_data_type' in ds.attrs:
            valid_cdm_types = ['TimeSeries', 'TimeSeriesProfile', 'Trajectory', 'TrajectoryProfile', 'Profile']
            if ds.attrs['cdm_data_type'] not in valid_cdm_types:
                result.errors.append(f"cdm_data_type must be one of {valid_cdm_types}, got '{ds.attrs['cdm_data_type']}'")
        ...
        if 'date_created' in ds.attrs:
            ...
            if not re.match(r'\d{8}T\d{6}$', date_str):
                result.errors.append(f"date_created must use format YYYYmmddTHHMMss, got '{date_str}'")
        
        # Check id format (should match filename pattern)
        if 'id' in ds.attrs:
            id_str = ds.attrs['id']
            if not re.match(r'^OS_[A-Z0-9]+_\d{8}-\d{8}_[A-Z]{3}_[A-Za-z0-9_T]+$', id_str):
                result.warnings.append(f"id should follow OceanSITES pattern OS_ARRAY_YYYYMMDD-YYYYMMDD_TYPE_CONTENT, got '{id_str}'")
    
    def _validate_data_values(self, ds: xr.Dataset, filepath: str, result: ValidationResult):
            ...
            if coord_name in ds.coords:
                ...
                if 'range' in coord_spec:
                    ...
                    if np.any((coord_vals < min_val) | (coord_vals > max_val)):
                        result.errors.append(f"{coord_name} values must be between {min_val} and {max_val}")
                ...
                # Check depth positive direction
                    ...
                    if positive == 'down':
                        ...
                        if np.any(coord_vals < min_val):
                            result.errors.append(f"DEPTH values must be ≥{min_val} when positive='down'")
                    elif positive == 'up' and np.any(coord_vals > 0):
                        result.errors.append("DEPTH values must be ≤0 when positive='up'")
        ...
        if 'TIME' in ds.coords:
            ...
            if 'units' in time_var.attrs:
                try:
                    ...
                    file_end = np.datetime64(end_date + timedelta(days=1))  # End of end date
                    
                    if np.any(time_values < file_start) or np.any(time_values >= file_end):
                        result.errors.append("TIME values must fall within filename date range")
                except Exception:
                    result.warnings.append("Could not validate TIME values against filename dates")
            ...
            if 'valid_min' in var.attrs and 'valid_max' in var.attrs:
                ...
                if len(valid_data) > 0:
                    if np.any(valid_data < valid_min) or np.any(valid_data > valid_max):
                        result.warnings.append(f"Variable {var_name} has values outside valid range [{valid_min}, {valid_max}]")
    ...
    return {
        ...
        'file_requirements': {
            'file_type_1': {
                ...
                'forbidden_dimensions': ['FORBIDDEN_DIM'],  # Dimensions that should not exist
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        ...
        print("To use different array specs, call validate_ac1_file() directly with array_specs parameter.")
        sys.exit(1)
