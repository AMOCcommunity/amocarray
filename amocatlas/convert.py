...
def enrich_contributor_ids(attrs: Dict, array_name: str = None) -> Dict:
    ...
    for name in names:
        ...
        if lookup_key in CONTRIBUTOR_ORCID_LOOKUP:
            ...
            logger.log_info(f"Found ORCID for {name} in {array_name}: {CONTRIBUTOR_ORCID_LOOKUP[lookup_key]}")
        else:
            # No match found, use empty string as placeholder
            orcids.append('')
            logger.log_warning(f"No ORCID found for contributor '{name}' in array '{array_name}'")
...
def to_AC1(
    ...
    if 'moc_transports' in source_file or _is_component_transport_data(ds):
        ac1_dataset = _convert_component_transports(ds, array_name, start_date, end_date)
...
def _is_component_transport_data(ds: xr.Dataset) -> bool:
    ...
    transport_vars = [var for var in ds.data_vars if 't_' in var or 'transport' in var.lower()]
...
def _convert_component_transports(
    ...
    transport_mapping = {
        't_gs10': {'name': 'Florida Straits', 'description': 'Florida Straits transport'},
        't_ek10': {'name': 'Ekman', 'description': 'Ekman transport'}, 
        't_umo10': {'name': 'Upper Mid-Ocean', 'description': 'Upper Mid-Ocean transport'},
        't_therm10': {'name': 'Thermocline', 'description': 'Thermocline recirculation 0-800m'},
        't_aiw10': {'name': 'Intermediate Water', 'description': 'Intermediate water 800-1100m'},
    ...
    available_transports = {var: transport_mapping[var] for var in transport_mapping if var in ds.data_vars}
    ...
    log.info(f"Found {n_components} transport components: {list(available_transports.keys())}")
    ...
    if 'units' not in orig_attrs:
        raise ValueError("TIME coordinate is missing units attribute. Units should be assigned in standardise.py")
    ...
    for var in component_names:
        ...
        if current_units is None:
            raise ValueError(f"Variable {var} is missing units attribute. Units should be assigned in standardise.py")
        
        # Check for acceptable transport units (should be standardized in standardise.py)
        acceptable_units = ['sverdrup', 'Sv', 'Sverdrup']  # Different acceptable formats
        if current_units not in acceptable_units:
            raise ValueError(f"Variable {var} has units '{current_units}' but expected one of {acceptable_units}. Units should be standardized in standardise.py")
    ...
    if 'moc_mar_hc10' in ds.data_vars:
        ...
        if current_units is None:
            raise ValueError(f"Variable moc_mar_hc10 is missing units attribute. Units should be assigned in standardise.py")
        
        if current_units not in acceptable_units:
            raise ValueError(f"Variable moc_mar_hc10 has units '{current_units}' but expected one of {acceptable_units}. Units should be standardized in standardise.py")
    ...
    transport_descriptions = [available_transports[var]['description'] for var in component_names]
    ...
    ac1_ds.attrs = _create_ac1_global_attributes(ds, array_name, start_date, end_date, 'transports_T12H')
...
def _create_ac1_global_attributes(
    ...
    # Title and summary
    attrs['title'] = f'{array_name} Atlantic Meridional Overturning Circulation Transport Components'
    attrs['summary'] = f'Component transport time series from the {array_name} observing array measuring the Atlantic Meridional Overturning Circulation.'
    ...
    contributor_attrs = [
        'contributor_name', 'contributor_email', 'contributor_id', 'contributor_role',
        'contributing_institutions', 'contributing_institutions_vocabulary', 
    ...
    if 'contributor_role_vocabulary' not in attrs:
        attrs['contributor_role_vocabulary'] = 'https://vocab.nerc.ac.uk/collection/W08/current/'
    
    # Source acknowledgement
    if 'acknowledgement' in ds.attrs:
        attrs['source_acknowledgement'] = ds.attrs['acknowledgement']
    elif array_name == 'RAPID':
        attrs['source_acknowledgement'] = 'Data from the RAPID AMOC observing project funded by the UK Natural Environment Research Council'
    ...
    attrs['processing_level'] = 'Data verified against model or other contextual information'
    attrs['comment'] = f'Converted to AC1 format from {ds.attrs.get("source_file", "original data")} using amocatlas.convert.to_AC1()'
...
def _get_ac1_filename(array_name: str, start_date: str, end_date: str, content_type: str, partx: str) -> str:
