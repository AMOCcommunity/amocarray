...
def valid_ac1_dataset():
    ...
    time_data = np.array(['2004-04-02T00:00:00', '2004-04-02T12:00:00', '2004-04-03T00:00:00'], dtype='datetime64[ns]')
    
    # Create 8 transport components as required
    component_names = ['Florida Straits', 'Ekman', 'Upper Mid-Ocean', 'Thermocline', 
                      'Intermediate Water', 'Upper NADW', 'Lower NADW', 'AABW']
...
def temp_netcdf_file(valid_ac1_dataset):
    ...
    proper_name = os.path.join(temp_dir, 'OS_RAPID_20040402-20040403_DPR_transports_T12H.nc')
    ...
    ds.encoding['TIME'] = {'units': 'seconds since 1970-01-01T00:00:00Z', 'calendar': 'gregorian'}
...
class TestComplianceChecker:
    ...
    def test_missing_required_global_attributes(self, valid_ac1_dataset):
        ...
        with tempfile.NamedTemporaryFile(suffix='_OS_RAPID_20040402-20040403_DPR_transports_T12H.nc', delete=False) as tmp:
            if 'units' in ds['TIME'].attrs:
                del ds['TIME'].attrs['units']
            ds.encoding['TIME'] = {'units': 'seconds since 1970-01-01T00:00:00Z', 'calendar': 'gregorian'}
            ...
            assert any('Missing required global attribute: title' in error for error in result.errors)
        
        os.unlink(tmp.name)
    
    def test_invalid_filename_pattern(self, valid_ac1_dataset):
        """Test detection of invalid filename patterns."""
        with tempfile.NamedTemporaryFile(suffix='_invalid_filename.nc', delete=False) as tmp:
            ds = valid_ac1_dataset.copy()
            if 'units' in ds['TIME'].attrs:
                del ds['TIME'].attrs['units']
            ds.encoding['TIME'] = {'units': 'seconds since 1970-01-01T00:00:00Z', 'calendar': 'gregorian'}
            ...
            assert not result.passed
            assert any('does not match OceanSITES pattern' in error for error in result.errors)
    ...
    def test_missing_variable_attributes(self, valid_ac1_dataset):
        ...
        with tempfile.NamedTemporaryFile(suffix='_OS_RAPID_20040402-20040403_DPR_transports_T12H.nc', delete=False) as tmp:
            ...
            assert not result.passed
            assert any('TRANSPORT missing required attribute: long_name' in error for error in result.errors)
    ...
    def test_datetime_coordinate_units_not_required(self, valid_ac1_dataset):
        ...
        with tempfile.NamedTemporaryFile(suffix='_OS_RAPID_20040402-20040403_DPR_transports_T12H.nc', delete=False) as tmp:
            ...
            time_units_errors = [error for error in result.errors if 'TIME' in error and 'units' in error]
            assert len(time_units_errors) == 0, f"TIME should not require manual units. Errors: {time_units_errors}"
    ...
    def test_unitless_variables_allowed(self, valid_ac1_dataset):
        ...
        with tempfile.NamedTemporaryFile(suffix='_OS_RAPID_20040402-20040403_DPR_transports_T12H.nc', delete=False) as tmp:
            ...
            name_units_errors = [error for error in result.errors if 'TRANSPORT_NAME' in error and 'units' in error]
            desc_units_errors = [error for error in result.errors if 'TRANSPORT_DESCRIPTION' in error and 'units' in error]
            
            assert len(name_units_errors) == 0, f"TRANSPORT_NAME should not require units. Errors: {name_units_errors}"
            assert len(desc_units_errors) == 0, f"TRANSPORT_DESCRIPTION should not require units. Errors: {desc_units_errors}"
    ...
    def test_invalid_data_mode(self, valid_ac1_dataset):
        ...
        with tempfile.NamedTemporaryFile(suffix='_OS_RAPID_20040402-20040403_DPR_transports_T12H.nc', delete=False) as tmp:
            ...
            assert not result.passed
            assert any("data_mode must be one of ['R', 'P', 'D']" in error for error in result.errors)
    ...
    def test_invalid_qc_indicator(self, valid_ac1_dataset):
        ...
        with tempfile.NamedTemporaryFile(suffix='_OS_RAPID_20040402-20040403_DPR_transports_T12H.nc', delete=False) as tmp:
            ...
            assert not result.passed
            assert any("QC_indicator must be one of" in error for error in result.errors)
    ...
    def test_invalid_date_format(self, valid_ac1_dataset):
        ...
        with tempfile.NamedTemporaryFile(suffix='_OS_RAPID_20040402-20040403_DPR_transports_T12H.nc', delete=False) as tmp:
            ...
            assert not result.passed
            assert any('date_created must use format YYYYmmddTHHMMss' in error for error in result.errors)
    ...
    def test_missing_conventions(self, valid_ac1_dataset):
        ...
        with tempfile.NamedTemporaryFile(suffix='_OS_RAPID_20040402-20040403_DPR_transports_T12H.nc', delete=False) as tmp:
            ...
            assert not result.passed
            assert any("Conventions must include 'OceanSITES-1.4'" in error for error in result.errors)
            assert any("Conventions must include 'ACDD-1.3'" in error for error in result.errors)
