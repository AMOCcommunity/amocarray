...
class TestNOAC47N:
    ...
    @patch('amocatlas.data_sources.noac47n.ReaderUtils.load_array_metadata_with_fallback')
    ...
    def test_read_47n_basic_functionality(self, mock_print_info, mock_attach_metadata,
                                          mock_load_metadata, mock_resolve_path):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_ds = xr.Dataset({
                'MOC_transport': (['TIME'], [15.2, 14.8, 16.1])
            })
    ...
    @patch('amocatlas.data_sources.noac47n.ReaderUtils.attach_metadata_with_tracking')
    @patch('amocatlas.data_sources.noac47n.ReaderUtils.print_loading_info')
    def test_read_47n_transport_only_flag(self, mock_print_info, mock_attach_metadata,
                                          mock_load_metadata, mock_resolve_path):
    ...
    @patch('amocatlas.data_sources.noac47n.ReaderUtils.attach_metadata_with_tracking')
    @patch('amocatlas.data_sources.noac47n.ReaderUtils.print_loading_info')
    def test_read_47n_track_added_attrs(self, mock_print_info, mock_attach_metadata,
                                        mock_load_metadata, mock_resolve_path):
    ...
    def test_read_47n_no_download_url_error(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch('amocatlas.data_sources.noac47n.ReaderUtils.load_array_metadata_with_fallback') as mock_metadata:
                mock_metadata.return_value = ({}, {})
                
                # Patch the FILE_URLS to not contain our test file
                with patch.dict('amocatlas.data_sources.noac47n.A47N_FILE_URLS', {}, clear=True):
    ...
    def test_read_47n_unsupported_file_type(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch('amocatlas.data_sources.noac47n.ReaderUtils.load_array_metadata_with_fallback') as mock_metadata:
                ...
                with pytest.raises(FileNotFoundError, match="No valid data files found|Local file not found"):
                    noac47n.read_47n(
                        source=temp_dir,
    ...
    def test_read_47n_integration_with_sample_data(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.dict('amocatlas.data_sources.noac47n.A47N_FILE_URLS', 
                           {"NOAC_AMOC.tab": f"file://{sample_file}"}, clear=True):
    ...
    @patch('amocatlas.data_sources.noac47n.utilities.resolve_file_path')
    @patch('amocatlas.data_sources.noac47n.ReaderUtils.load_array_metadata_with_fallback')
    def test_read_47n_dataframe_conversion_error(self, mock_load_metadata, mock_resolve_path):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(ValueError, match="Failed to convert DataFrame to xarray Dataset"):
                noac47n.read_47n(
                    source="https://example.com/",
                    file_list=["NOAC_AMOC.tab"],
                    data_dir=temp_dir
